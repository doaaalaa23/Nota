...
class NotificationRepositoryImpl(NotificationRepository):
    ...
    def read_due_today(self, today: date) -> List[UserNotification]:
        ...
        for contract, client_name in contracts:
            # Guard: a contract with no installment plan set yet must not crash the run.
            if not contract.installment_num or not contract.first_installment:
                continue
            ...
            # NOTE: this assumes payments are always made in exact multiples of
            # installment_value. If partial payments are possible in your flow,
            # this will under/over-count how many installments are "paid" and
            # can suppress or duplicate a notification. Track paid installments
            # by index/id instead of back-computing from a sum if that's the case.
